...
-- version
DEF sqld360_vYYNN = 'v203';
DEF sqld360_vrsn = '&&sqld360_vYYNN. (2022-02-16)';
 ...
 BEGIN
  SELECT parsing_schema_name
   ...
   WHERE sql_id =  '&&sqld360_sqlid.'
     ...
     AND '&&license_pack.'<>'N';
 EXCEPTION WHEN NO_DATA_FOUND THEN
  -- pick any user that executed the SQL
  -- this search if non-determistic just for the sake of performance.
  BEGIN
   SELECT username
     INTO :xplan_user  
     FROM dba_users
  ...
  EXCEPTION
   ...
   -- will try our luck with the current schema.
    :xplan_user:='&&current_user.';
  END;
 END;
END;
/
...
SET LONG 1000000000 LONGC 4000
...
SELECT CASE WHEN '&&exact_matching_signature.' = '&&force_matching_signature.' THEN '--' END skip_force_match
  FROM DUAL
/
-- Due to some unknown bug sometimes the signatures are 0 in GV$/DBA views and in order to keep the integrity of the search the columns' NEW_V are turned off
-- as any query in the future, specially in 2b, that fetches those columns will replace the values generated by DBMS_SQLTUNE with 0 and queries after 2b get mess up. 
COL exact_matching_signature OFF
COL force_matching_signature OFF
