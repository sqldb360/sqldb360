...
----  IF data_already_loaded = 'N' THEN
    ...
    INSERT INTO plan_table (statement_id,timestamp,remarks,                -- 'SQLD360_ASH_DATA', sample_time, sql_id
                            ...
                            &&skip_ver_le_10.distribution,                       -- sql_exec_start
     ...
     SELECT 'SQLD360_ASH_DATA_HIST', sample_time, sql_id, 
            snap_id, dbid,
            &&skip_ver_le_10.sql_plan_operation, 
            &&skip_ver_le_10.sql_plan_options, 
            ...
            NVL(wait_class,'CPU'),
            &&skip_ver_le_10.sql_plan_line_id, 
            &&skip_ver_le_10.sql_exec_id,
            &&skip_ver_le_10.TO_CHAR(sql_exec_start,'YYYYMMDDHH24MISS'),
            session_id, session_serial#, 
            sample_id,
            seq#||','||p1text||','||p1||','||p2text||','||p2||','||p3text||','||p3||','||current_file#||','||current_block#||
            ','||&&skip_ver_le_10.current_row#||
            ','||&&skip_ver_le_11_1.tm_delta_time||
            ','||&&skip_ver_le_11_1.tm_delta_cpu_time||
            ','||&&skip_ver_le_11_1.tm_delta_db_time||
            ','||user_id
            ,
            &&skip_ver_le_10.in_parse||
            ','||
            &&skip_ver_le_10.in_hard_parse||
            ','||
            &&skip_ver_le_10.in_sql_execution||
            ','||qc_instance_id||','||qc_session_id||','||
            &&skip_ver_le_10.qc_session_serial#||
            ','||blocking_session_status||','||blocking_session||','||blocking_session_serial#||','||
            &&skip_ver_le_11_1.blocking_inst_id||
            ','||&&skip_ver_le_11_201.px_flags||
            ','||&&skip_ver_le_11_1.pga_allocated||
            ','||&&skip_ver_le_11_1.temp_space_allocated||
            ','||&&skip_ver_le_11_1.delta_time||
            ','||&&skip_ver_le_11_1.delta_read_io_requests||
            ','||&&skip_ver_le_11_1.delta_write_io_requests||
            ','||&&skip_ver_le_11_1.delta_read_io_bytes||
            ','||&&skip_ver_le_11_1.delta_write_io_bytes||
            ','||&&skip_ver_le_11_1.delta_interconnect_io_bytes||
            ','&&skip_ver_le_12_101.||sql_full_plan_hash_value
