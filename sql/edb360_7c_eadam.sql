...
SPO &&cdb_awr_object_prefix.xtr_control.txt;
...
SPO OFF;
HOS gzip &&cdb_awr_object_prefix.xtr_control.txt
HOS tar -cf &&edb360_tar_filename..tar &&cdb_awr_object_prefix.xtr_control.txt.gz
HOS rm &&cdb_awr_object_prefix.xtr_control.txt.gz

/* ------------------------------------------------------------------------- */

SPO &&cdb_object_prefix.tab_columns.txt;
SELECT table_name,
       column_id,
       column_name,
       data_type,
       ...
       data_precision,
       data_scale
  FROM &&cdb_object_prefix.tab_columns
 WHERE (owner, table_name) IN 
((UPPER('&&tool_repo_user.'), '&&cdb_awr_hist_prefix.ACTIVE_SESS_HISTORY')
)
ORDER BY CASE owner WHEN UPPER('&&tool_repo_user.') THEN 1 ELSE 2 END, table_name, column_id;
SPO OFF;
HOS gzip &&cdb_object_prefix.tab_columns.txt
HOS tar -rf &&edb360_tar_filename..tar &&cdb_object_prefix.tab_columns.txt.gz
HOS rm &&cdb_object_prefix.tab_columns.txt.gz

/* ------------------------------------------------------------------------- */

SPO &&cdb_awr_hist_prefix.active_sess_history.txt;
SELECT /*+ &&ds_hint.
...
SPO OFF;
HOS gzip &&cdb_awr_hist_prefix.active_sess_history.txt
HOS tar -rf &&edb360_tar_filename..tar &&cdb_awr_hist_prefix.active_sess_history.txt.gz
HOS rm &&cdb_awr_hist_prefix.active_sess_history.txt.gz
